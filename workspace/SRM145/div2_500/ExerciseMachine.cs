...
public class ExerciseMachine {
	public int getPercentages(string time) {
        ...
        int hour = int.Parse(strArray[0]);
        int minute = int.Parse(strArray[1]);
        int second = int.Parse(strArray[2]);
...
// CUT begin
	static bool DoTest(string time, int __expected) {
	...
	public static void Main(string[] args){
		...
		Console.Error.WriteLine("ExerciseMachine (500 Points)");
		Console.Error.WriteLine();

    	int nCases = 0, nPassed = 0;
    	using (var reader = File.OpenText("ExerciseMachine.sample")) {
            while (true) {
                string line = reader.ReadLine();
                if (line == null || !line.StartsWith("--"))
                    break;

                string time = (string) Convert.ChangeType(reader.ReadLine(), typeof(string));
                reader.ReadLine();
		...
		int T = (int)((DateTime.UtcNow - Jan1st1970).TotalSeconds - 1514133707);
